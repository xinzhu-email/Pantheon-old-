...
class FlowPlot:
    def __init__(self, data, color_map, x_init_idx = 0, y_init_idx = 0, allow_select = True, select_color_change = True, legend = None, main_plot = None):
        self.adata = data
    ...
    def gate_func(self):
        self.view.filters = [IndexFilter(self.source.selected.indices)]
...
class CreateTool:
    ...
    def base_tool(self):
        ...
        layout=row(column(Figure.p, Figure.show_gene_list, Figure.para_color, Figure.trigger_color),
    ...
    def highlight_gene(self,main_plot):
        hl_figure = FlowPlot(data=self.adata, color_map=self.hl_gene_map, main_plot=main_plot)
...
class connect_figure():
    ...
    def get(
        ...
        #Figure, main  = CreateTool().base_tool()
        if selected_color:      
            return getattr(self.fig.color_selection,'color')
        if checked_class:
            return getattr(self.fig.class_checkbox,'active')
        if selected_group:
            return getattr(self.fig.group,'value')
        if data:
            return self.fig.adata
        if selected_indices:
            return self.fig.source.selected.indices
        if data_color:
            return self.fig.source.data['color']
    ...
    def set_change(
        ...
        if data_color is not None:
            self.fig.source.data['color'] = data_color
            print(self.fig.source.data['color'])
        if filter_cells is not None:
            boolean_index = self.fig.adata.obs.index.isin(list(filter_cells))
            print(boolean_index)
            indices = self.fig.adata.obs[self.fig.adata.obs.index.isin(list(filter_cells))]['ind']
            self.fig.view.filters = [IndexFilter(indices)]
    ...
    def add_layout(
        ...
        if button:
            layout = Button(label=label)
            
            layout.on_click(lambda: callback_function(*parameter))
            print('====',layout,'=====')
            return layout
        if input:
            layout = TextInput(title=label, value='')
            return layout
